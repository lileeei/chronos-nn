...
impl RnnCell {
    ...
    /// # Returns
    ///
    /// * `h_next` - 当前时间步计算出的新隐藏状态。
    pub fn forward(&self, x_t: &Array1<f64>, h_prev: &Array1<f64>) -> Array1<f64> {
        // h_t = tanh(W_hh * h_{t-1} + W_ih * x_t + b_h)
        let h_next = self.w_hh.dot(h_prev) + self.w_ih.dot(x_t) + &self.b_h;
        tanh(&h_next)
    }
}
...
mod tests {
    ...
    fn test_rnn_cell_forward_pass() {
        let input_size = 10;
        let hidden_size = 20;
        let cell = RnnCell::new(input_size, hidden_size);
        ...
        // 执行前向传播
        let h_next = cell.forward(&x_t, &h_prev);
